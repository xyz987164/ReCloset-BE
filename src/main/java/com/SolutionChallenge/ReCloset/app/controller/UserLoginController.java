...
import com.SolutionChallenge.ReCloset.app.dto.LoginRequestDto;
import com.SolutionChallenge.ReCloset.app.service.LoginService;
import com.SolutionChallenge.ReCloset.global.dto.ApiResponseTemplete;
import com.SolutionChallenge.ReCloset.app.dto.LoginResponseDto;
...
*/
    ...
    @Operation(summary = "구글 소셜 로그인 API (토큰 인증 불필요) \n 소셜 로그인 창으로 이동 후 /login/oauth2/code/{registerationid} 로 리디렉션")
    ...
    public RedirectView redirectToGoogle() {
        ...
        String prompt = "consent";  // 항상 동의 절차를 강제

        // access_type=offline은 별도의 파라미터로 추가
        String url = String.format("%s?client_id=%s&redirect_uri=%s&response_type=%s&scope=%s&access_type=offline&prompt=%s",
                AUTH_URL, clientId, redirectUri, responseType, scope, prompt);
